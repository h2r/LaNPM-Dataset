...
def grasp_object(robot_state_client, manipulation_api_client, pick_pixels, image):

    # specify the pick location (coordinates) in the image
   ...
   # Request Pick Up on that pixel.
    ...
    #check for success of execution + wait for 10 seconds after request
    success = False; start = time.time()
...
def arm_object_grasp(robot_state_client, manipulation_api_client, pick_pixels,image):
    ...
    # We can specify where in the gripper we want to grasp. About halfway is generally good for
    # small objects like this. For a bigger object like a shoe, 0 is better (use the entire gripper)
    grasp.grasp_params.grasp_palm_to_fingertip = 0.6
