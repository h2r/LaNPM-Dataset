...
def simple_move_to(robot, position, rotation_matrix, end_time=30):
    ...
    move_cmd = RobotCommandBuilder.trajectory_command(
        goal_x=position[0],
        goal_y=position[1],
        goal_heading=math_helpers.Quat.from_matrix(rotation_matrix).to_yaw(),
    ...
    block_for_trajectory_cmd(command_client, cmd_id,
                             timeout_sec=10.0, verbose=True)


def move_to(robot, robot_state_client, pose=None, distance_margin=1.00, hostname="138.16.161.12", end_time=30):
    robot.logger.info("Powering on robot... This may take a several seconds.")
    robot.power_on(timeout_sec=20)
