...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for batch, train_batch in enumerate(train_dataloader):
            ...
            for idx in range(0, batch_steps, args.train_subbatch):
                ...
                observations = {
                    "image": train_batch[0][idx : min(idx + args.train_subbatch, batch_steps)],
                    "context": get_text_embedding(train_batch[1][idx : min(idx + args.train_subbatch, batch_steps)]),
                }

            
                actions = {
                    'terminate_episode': train_batch[2][idx : min(idx + args.train_subbatch, batch_steps)],
                    'pickup_release': train_batch[3][idx : min(idx + args.train_subbatch, batch_steps)],
                    'body_position_delta': train_batch[4][idx : min(idx + args.train_subbatch, batch_steps)],
                    'body_yaw_delta': train_batch[5][idx : min(idx + args.train_subbatch, batch_steps)],
                    'body_pitch_delta': train_batch[6][idx : min(idx + args.train_subbatch, batch_steps)],
                    'arm_position_delta': train_batch[7][idx : min(idx + args.train_subbatch, batch_steps)],
                    'control_mode': train_batch[8][idx : min(idx + args.train_subbatch, batch_steps)]
                }

                padding = train_batch[9][idx : min(idx + args.train_subbatch, batch_steps)]
